export const dict = [

  { pt : 
    {
      introTitle: "Desenvolvedor Web Júnior",
      introDescription: "Tenho muito orgulho da minha história. Antes de me formar em <strong>Desenvolvimento Web pela Trybe</strong> [com mais de 60 projetos concluídos], fiz algumas coisas. Fui <strong>baterista de hardcore</strong>/punk/metal, me formei em publicidade pela <strong>Cásper Líbero</strong> [carreira que segui até me tornar <strong>coordenador de área</strong> da uma grande agência], andei <strong>pela África do Sul</strong>, tive um <strong>hostel na Praia do Rosa-SC</strong>, co-criei um <strong>aplicativo premiado de entretenimento adulto</strong>, fiz uma <strong>viagem de moto até Alasca e Ushuaia</strong> e abri uma <strong>pousada em Pipa RN</strong>.",
      introKnowMeBetter: "saiba +",
      introAltText: "Este sou eu no jornal local, após viajar mais de 135 mil kms de moto do Brasil ao Alasca EUA ao Ushuaia Argentina",
      stacks: "Tecnologias/ Linguagens",
      titleProjets: "Projetos",
      more: "ver mais ->",
      certificates: "Certificados",
    }
  },

  { en: 
    {
      introTitle: "Junior Web Developer",
      introDescription: "I'm very proud of my story. Before being a <strong> Web Developer graduated from Trybe</strong> with more than 60 completed projects, I did a few things. I was a <strong>hardcore/punk/metal drummer </strong>, I graduated in advertising from <strong>Cásper Líbero</strong>, a career that I followed until I became <strong>area coordinator</strong> of a large agency, I walked <strong>through South Africa </strong>, I had a <strong> hostel in Praia do Rosa-SC</strong>, I co-created an <strong>award-winning adult entertainment app</strong>, I took a <strong>motorcycle trip to Alaska and Ushuaia</strong>, I opened a <strong>guesthouse in Pipa RN</strong>, I became a surfer and I wasn't afraid to make another <strong>career transition</strong>.",
      introKnowMeBetter: "know me +",
      introAltText: "This is me in the local newspaper, after traveling more than 135 thousand km by motorcycle from Brazil to Alaska USA to Ushuaia Argentina",
      stacks: "Stacks/ Frameworks",
      titleProjets: "Projects",
      more: "checkout more ->",
      certificates: "Certificates",
    }
  },

  { es: 
    {
      introTitle: "Desarrollador Web Júnior",
      introDescription:"Estoy muy orgulloso de mi historia. Antes de ser un <strong>desarrollador web graduado de Trybe</strong> con más de 60 proyectos completados, hice algunas cosas. Era un <strong>hardcore/punk/ baterista de metal</strong>, me gradué en publicidad en <strong>Cásper Líbero</strong>, carrera que seguí hasta convertirme en <strong>coordinador de área</strong> de una gran agencia, caminé por Sudáfrica </strong>, tenía un <strong> albergue en Praia do Rosa-SC</strong>, co-creé una <strong>aplicación de entretenimiento para adultos galardonada</strong>, tomé una <strong> viaje en moto a Alaska y Ushuaia</strong>, abrí una <strong>casa de huéspedes en Pipa RN</strong>, me convertí en surfista y no tuve miedo de hacer otra <strong>transición profesional</strong>.", 
      introKnowMeBetter: "sepa +",
      introAltText: "Este soy yo en el periódico local, después de recorrer más de 135 mil km en moto desde Brasil hasta Alaska USA hasta Ushuaia Argentina",
      stacks: "Tecnologias/ Lenguages",    
      titleProjets: "Proyetos",
      more: "ver más ->",
      certificates: "Certificaciones",
    }
  }
]
